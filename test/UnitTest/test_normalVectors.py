...
class TestFindScaledNormales:
    ...
    def test_normals_count_matches_neighbors(self):
        ...
        assert len(tri1.scaled_normal) == len(tri1.ngb), \
    ...
    def test_normals_perpendicular_to_edges(self):
        ...
        for i, ngb_id in enumerate(tri1.ngb):
            ...
            # Find shared edge
            self_pts = set(tuple(p) for p in tri1.cords)
            ngb_pts = set(tuple(p) for p in ngb.cords)
            shared_pts = list(self_pts & ngb_pts)
    ...
    def test_normals_point_outward(self):
        ...
        for i, ngb_id in enumerate(tri1.ngb):
            ...
            to_midpoint = np.array([
                tri1.midpoint[0] - edge_mid[0],
                tri1.midpoint[1] - edge_mid[1]
            ], dtype=float)
            ...
            assert dot <= 1e-9, \
                f"Normal {i} does not point outward: dot={dot}"

    def test_normals_toward_neighbor(self):
        ...
        for i, ngb_id in enumerate(tri1.ngb):
            ...
            to_neighbor = np.array([
                ngb.midpoint[0] - edge_mid[0],
                ngb.midpoint[1] - edge_mid[1]
            ], dtype=float)
            ...
            assert dot >= -1e-9, \
                f"Normal {i} does not point toward neighbor: dot={dot}"

    def test_normals_ordered_match_neighbors(self):
    ...
    def test_single_triangle_with_one_neighbor(self):
        ...
        assert len(tri1.scaled_normal) == 1, f"tri1 should have 1 normal, got {len(tri1.scaled_normal)}"
        assert len(tri2.scaled_normal) == 1, f"tri2 should have 1 normal, got {len(tri2.scaled_normal)}"
